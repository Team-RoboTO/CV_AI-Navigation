...
class BboxWithDistance(Node):
    ...
    def detections_callback(self, detections_msg, img_msg):
        ...
        cv2_img_flip = cv2.flip(cv2_img, -1)  # -1 per flip di 180°
        ...
        for detection in detections_msg.detections:
            ...
            # FLIP le coordinate di 180°
            flipped_center_x = img_width - center_x -200
            flipped_center_y = img_height - center_y -200
            ...
            # Usa le coordinate flippate
            min_pt = (round(flipped_center_x - (width / 2.0)),
                      round(flipped_center_y - (height / 2.0)))
            max_pt = (round(flipped_center_x + (width / 2.0)),
                      round(flipped_center_y + (height / 2.0)))
            
            new_min_pt = (round(flipped_center_x - s_x),
                          round(flipped_center_y - s_y))
            new_max_pt = (round(flipped_center_x + s_x),
                          round(flipped_center_y + s_y))
            ...
            cv2.rectangle(cv2_img_flip, min_pt, max_pt,
                          self.color, self.bbox_thickness)
            cv2.rectangle(cv2_img_flip, new_min_pt, new_max_pt,
                          self.color, self.bbox_thickness)
            cv2.putText(cv2_img_flip, label, (min_pt[0], min_pt[1]-2 if outside else min_pt[1]+h+2),
                        0, lw / 3, txt_color, thickness=tf, lineType=cv2.LINE_AA)
            cv2.putText(cv2_img_flip, coordinates, (min_pt[0], max_pt[1]+3*(tf+lw) if outside else max_pt[1]+h+2),
                        0, lw / 3, txt_color, thickness=tf, lineType=cv2.LINE_AA)

        processed_img = self._bridge.cv2_to_imgmsg(
            cv2_img_flip, encoding=img_msg.encoding)
