  ...
  dist_coeffs_(cv::Mat(1, static_cast<int>(dist_coeffs.size()), CV_64F, const_cast<double *>(dist_coeffs.data())).clone())
{
  ...
  small_armor_points_.emplace_back(cv::Point3f(0,  small_half_y, -small_half_z));  // BL
  small_armor_points_.emplace_back(cv::Point3f(0,  small_half_y,  small_half_z));  // TL
  small_armor_points_.emplace_back(cv::Point3f(0, -small_half_y,  small_half_z));  // TR
  small_armor_points_.emplace_back(cv::Point3f(0, -small_half_y, -small_half_z));  // BR

  // Build 4-point model for LARGE armor (same ordering, different dimensions)
  large_armor_points_.emplace_back(cv::Point3f(0,  large_half_y, -large_half_z));  // BL
  large_armor_points_.emplace_back(cv::Point3f(0,  large_half_y,  large_half_z));  // TL
  large_armor_points_.emplace_back(cv::Point3f(0, -large_half_y,  large_half_z));  // TR
  large_armor_points_.emplace_back(cv::Point3f(0, -large_half_y, -large_half_z));  // BR
}
...
bool PnPSolver::solvePnP(const Armor & armor, cv::Mat & rvec, cv::Mat & tvec)
{
  ...
  return cv::solvePnP(
    object_points, image_armor_points, camera_matrix_, dist_coeffs_, rvec, tvec,
...
{
  float cx = camera_matrix_.at<double>(0, 2);  // principal point X [px]
  float cy = camera_matrix_.at<double>(1, 2);  // principal point Y [px]
