...
{
  this->x_pri = this->x_post;
  this->P_pri = this->P_post;
}
...
void ExtendedKalmanFilter::inflateCovariance(int idx, double factor)
{
  ...
  this->P_post.row(idx) *= s;
  this->P_post.col(idx) *= s;
  this->P_post = ((this->P_post + this->P_post.transpose()) * 0.5).eval();   // .eval() to force immediate computation and prevent aliasing issues!
}
...
{
  this->P_post.row(idx).setZero();
  this->P_post.col(idx).setZero();
  this->P_post(idx, idx) = variance;
  this->P_pri.row(idx).setZero();
  this->P_pri.col(idx).setZero();
  this->P_pri(idx, idx) = variance;
}
...
Eigen::VectorXd ExtendedKalmanFilter::predict()
{
  ...
  this->F = this->jacobian_f(this->x_post), this->Q = this->update_Q();

  // State prediction: apply the nonlinear process model (damped constant-velocity)
  this->x_pri = this->f(this->x_post);
  ...
  this->P_pri = ((this->P_pri + this->P_pri.transpose()) * 0.5).eval();
  ...
  if (this->max_covariance.size() == this->n) {
    for (int i = 0; i < this->n; i++) {
      double cov_max = std::max(this->max_covariance(i), 1e-6);  // guard against zero/negative
      if (this->P_pri(i, i) > cov_max) {
        // Scale factor: sqrt(max / current) applied to row+col gives diagonal = max
        double scale = std::sqrt(cov_max / std::max(this->P_pri(i, i), 1e-10));
        this->P_pri.row(i) *= scale;
        this->P_pri.col(i) *= scale;
      }
    }
  }
  ...
  if (this->max_condition_number > 0.0) {
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> es(this->P_pri, Eigen::EigenvaluesOnly);
    if (es.info() == Eigen::Success) {
      double lmin = es.eigenvalues().minCoeff();
      double lmax = es.eigenvalues().maxCoeff();
      if (lmin < 1e-10 || lmax / std::max(lmin, 1e-15) > this->max_condition_number) {
        this->P_pri = 0.7 * this->P_pri + 0.3 * this->P0_;
        this->P_pri = (this->P_pri + this->P_pri.transpose()) * 0.5;
      }
    }
  }
  ...
  const Eigen::VectorXd & z, const Eigen::VectorXd & x_linearize)
{
  ...
  Eigen::MatrixXd H_tmp = this->jacobian_h(x_linearize);
  Eigen::MatrixXd R_tmp = this->update_R(z);
  Eigen::MatrixXd S = H_tmp * this->P_pri * H_tmp.transpose() + R_tmp;
  Eigen::VectorXd y = z - this->h(this->x_pri);
  Eigen::LDLT<Eigen::MatrixXd> S_ldlt(S);
...
Eigen::VectorXd ExtendedKalmanFilter::update(const Eigen::VectorXd & z)
{
  ...
  this->H = this->jacobian_h(this->x_pri), this->R = this->update_R(z);

  // Innovation covariance: S combines prediction uncertainty and sensor noise
  Eigen::MatrixXd S = this->H * this->P_pri * this->H.transpose() + this->R;
  ...
  this->K = S_ldlt.solve(this->H * this->P_pri.transpose()).transpose();
  ...
  this->x_post = this->x_pri + this->K * (z - this->h(this->x_pri));
  ...
  Eigen::MatrixXd IKH = this->I - this->K * this->H;
  this->P_post = IKH * this->P_pri * IKH.transpose() + this->K * this->R * this->K.transpose();
  ...
  this->P_post = ((this->P_post + this->P_post.transpose()) * 0.5).eval();
  ...
  const Eigen::MatrixXd & R_ext)
{
  ...
  Eigen::MatrixXd S = H_ext * this->P_post * H_ext.transpose() + R_ext;
  ...
  Eigen::MatrixXd K_ext = S_ldlt.solve(H_ext * this->P_post.transpose()).transpose();

  // State correction
  this->x_post = this->x_post + K_ext * (z - z_pred);

  // Joseph form covariance update
  Eigen::MatrixXd IKH = this->I - K_ext * H_ext;
  this->P_post = IKH * this->P_post * IKH.transpose() + K_ext * R_ext * K_ext.transpose();
