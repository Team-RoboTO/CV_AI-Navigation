  ...
  double muzzle_speed) const
{
  ...
  for (int i = 0; i < kMaxIter; ++i) {
    const double dz = 0.5 * params_.gravity * t * t;
    pitch = std::atan2(target_z + dz, ground_dist);
    pitch = std::clamp(pitch, params_.pitch_min, params_.pitch_max);
    ...
    if (params_.use_quadratic_drag) {
      ...
      const double k_q = params_.quadratic_drag_coeff;
      if (k_q < 1e-6) {
      ...
      const double k_l = params_.linear_drag_coeff;
      if (k_l < 1e-6) {
        new_t = ground_dist / v_h;
      } else {
  ...
  const double unclamped_pitch =
    std::atan2(target_z + 0.5 * params_.gravity * t * t, ground_dist);
  ...
  out.reachable =
    unclamped_pitch >= params_.pitch_min &&
    unclamped_pitch <= params_.pitch_max;
