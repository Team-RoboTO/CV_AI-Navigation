...
Topics:
    /detector/armors              vision_msgs/Detection2DArray   legacy bbox output
    /detector/armors_keypoints    std_msgs/String                bbox + keypoints JSON
    /yolo/debug_image             sensor_msgs/Image              raw ZED left image
...
from auto_aim_2.msg import ArmorKeypoint, ArmorKeypointArray
...
class Yolo26PoseKeypointsNode(Node):
    def __init__(self):
        ...
        # Typed keypoint output consumed by auto_aim_2.
        ...
        self.zed.set_camera_settings(sl.VIDEO_SETTINGS.GAIN, 80)
        ...
        while self.running:
            ...
            if boxes is not None and len(indices) > 0:
                for raw_i in np.array(indices).flatten()[:MAX_DETECTIONS]:
                        ...
                        # Legacy bbox topic for current auto_aim compatibility.
                        det = Detection2D()
                        ...
                        # New typed keypoint output for auto_aim_2.
                        kmsg = ArmorKeypoint()
