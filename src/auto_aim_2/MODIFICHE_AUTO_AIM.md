...
colcon build --packages-select auto_aim_2 --symlink-install
...
C++ files (`tracker.cpp`, `auto_aim_node.cpp`, `pnp_solver.cpp`) still require
a rebuild after each change.

## Launch

```bash
ros2 launch auto_aim_2 auto_aim_launch.py
```
...
- `auto_aim_2` — main C++ node (subscribes to detector, publishes `/cmd_vel_AI`).
- `auto_aim_viewer` — debug overlay (publishes `/tracker/debug_image`).
...
|---|---|---|
| `target_classes` | `['0']` | Which class IDs the auto-aim attacks. `0`=blue, `3`=red. Set to YOUR ENEMIES' color. |
...
2. **target_classes is wrong.** If the enemies are red (class `3`) but
   `target_classes: ['0']`, every detection is filtered out. Check enemy color
   and update. In the launch file:
   ```python
   {'target_classes': ['3']},   # red enemies
   {'target_classes': ['0']},   # blue enemies
   {'target_classes': ['0','3']},  # any color (training/mixed match)
   ```
...
3. **Message type not built.** If the viewer prints "ArmorKeypointArray not
   available", rebuild the package: `colcon build --packages-select auto_aim_2`.

---

# PART 4 — Full change history

This is what was changed from the original `auto_aim_2_final.zip` you started
...
# install/auto_aim_2/share/auto_aim_2/launch/auto_aim_launch.py
# (only works if you built with --symlink-install; otherwise edit in src/)

# DETECTOR
{'target_classes': ['0']},      # blue=0, red=3 — match enemy color
