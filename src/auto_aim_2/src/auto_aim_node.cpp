...
#include <auto_aim_2/msg/armor_keypoint_array.hpp>
...
public:
  ...
  : Node("auto_aim", options)
  {
    ...
    // Target classes to track ("3" = red, "0" = blue)
    ...
    target_classes_.erase("1");  // never track grey/dead
    ...
    // ── Subscribers ──
    if (use_keypoints_) {
      kpt_sub_ = create_subscription<auto_aim_2::msg::ArmorKeypointArray>(
...
private:
  ...
  void keypointCallback(const auto_aim_2::msg::ArmorKeypointArray::ConstSharedPtr msg)
  {
    if (!pnp_.ready()) {
      return;
    }
  ...
  void publishCommand(const std_msgs::msg::Header & header, const AimResult & aim)
  {
    ...
    // Some micro firmwares report pitch with opposite sign from the command
    // convention (+ = down in feedback while command may be - for the same pose).
    // Do not change the command sign; only normalize feedback for lock/viewer logic.
    const double micro_pitch_for_lock =
      micro_pitch_feedback_opposite_sign_ ? -micro_pitch_raw_ : micro_pitch_raw_;
  ...
  rclcpp::Subscription<auto_aim_2::msg::ArmorKeypointArray>::SharedPtr kpt_sub_;
