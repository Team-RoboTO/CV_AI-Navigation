...
private:
  ...
  {
    // Input
    declare_parameter("input_mode", std::string("zed_trt"));  // zed_trt | ros_topics
  ...
  void process_frame(const std::vector<ArmorPx> & dets, TimePoint t_capture)
  {
    ...
    GimbalSample gs_cap;
    if (!gimbal_buf_.query(t_capture, gs_cap)) return;  // no micro yet
    ...
    if (enemy >= 0) {
      for (const auto & d : dets) {
        if (d.class_id != enemy) continue;   // grey (1) and own color skipped
        if (d.confidence < min_confidence_) continue;
        auto aw = solver_->solve(d, gs_cap.yaw, gs_cap.pitch, ego);
        if (!aw) continue;
        if (aw->pos_cam.norm() > max_armor_dist_) continue;
        armors.push_back(*aw);
      }
    }
    ...
    if (debug_enable_ && (frame_count_++ % debug_every_ == 0)) {
      publish_debug(dbg, armors.size(), t_now, t_capture);
    }
  }
  ...
  // ────────────────────────────── debug ───────────────────────────────────
  void publish_debug(const AimDebug & d, size_t n_armors, TimePoint t_now,
                     TimePoint t_capture)
  {
  ...
  bool debug_enable_ = false, debug_markers_ = false;
  int debug_every_ = 4;
